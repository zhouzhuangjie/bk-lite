...
def get_signed_url(params, ak, secret, accept_format, method, body_params, signer=mac1, header=None):
    if header is None:
        header = {}
    url_params = __refresh_sign_parameters(params, ak, accept_format, signer)
    sign_params = dict(url_params)
    sign_params.update(__update_body_sign_params(body_params, header))
...
def __update_body_sign_params(body_params, header):
    sign_body_params = {}
    if "x-ascm-sign-version" not in header.keys():
        return body_params

    ascm_sign_version = header["x-ascm-sign-version"]
    if ascm_sign_version == "1.0":
        sign_body_params.update(body_params)
        for key, value in body_params.items():
            # for nodejs asapi signature compatibility
            if type(value).__name__ == "dict":
                sign_body_params[key] = "[object Object]"
            elif type(value).__name__ == "list":
                sign_body_params.pop(key)
                for i in range(len(value)):
                    if type(value[i]).__name__ == "dict" or type(value[i]).__name__ == "list":
                        sign_body_params[key + "." + str(i + 1)] = "[object Object]"
                    else:
                        sign_body_params[key + "." + str(i + 1)] = value[i]

        return sign_body_params
    else:
        return body_params
