...
from aliyunsdkcore.vendored.requests import Request, Session, status_codes
...
class HttpResponse(HttpRequest):
    ...
    def do_http_debug(self, request, response):
        # logger the request
        request_base = "\n> {} {} HTTP/1.1".format(self.get_method().upper(), self.get_url())
    ...
    def get_response_object(self):
        with Session() as s:
            ...
            host = self.get_host()
            if host.startswith("https://") or not host.startswith("https://") and current_protocol == "https://":
